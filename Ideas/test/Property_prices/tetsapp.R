...
server <- function( input, output, session ){
  ...
  shiny::observeEvent( input$myMap_shape_click, {
    ...
    #   
    # } else 
      if((click$id %in% lines.of.interest@data$community) & !any(duplicated(click.list$ids) & click.list$ids == click$id)==F ){
        browser()
      ...
      {
        browser()
        nameMatch <- lines.of.interest@data$community[lines.of.interest@data$community == click$id]
        click.list$ids <- c(click.list$ids, click.list$ids[click.list$ids %in% click$id] )
        click.list$ids <- c(click.list$ids, click.list$ids[click.list$ids %in% nameMatch] )
...
# # Create a list
# my_list <- list("apple", "banana", "cherry", "apple", "date", "banana")
# 
# # Element to check for uniqueness
# element_to_check <- "banana"
# 
# # Check if the element is unique in the list
# is_unique <- !any(duplicated(my_list) & my_list == element_to_check)
# 
# # Print the result
# if (is_unique) {
#   cat(paste(element_to_check, "is unique in the list.\n"))
# } else {
#   cat(paste(element_to_check, "is not unique in the list.\n"))
# }
